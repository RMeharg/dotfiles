...

{
  ...
  if [ "$(/usr/bin/basename "${0}")" = '.run.sh' ]; then
    ...
    /usr/bin/curl --progress-bar --location 'https://github.com/rmmeharg/.dotfiles/archive/master.tar.gz' | /usr/bin/tar -x --strip-components 1
  fi

  # Run scripts.
  /bin/bash ${dotfiles_dir}/.install-mac
